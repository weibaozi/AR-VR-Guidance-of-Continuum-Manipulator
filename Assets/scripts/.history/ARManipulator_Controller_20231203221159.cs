...
public class ARManipulator_Controller : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        foreach (var link in LinkObjects) {
            ...
            // rotationZ=
            print("link z: " + Mathf.Abs(180-link.transform.localRotation.eulerAngles.z),Mathf.Abs(180-link.transform.localRotation.eulerAngles.z) > (180-maxAngel));
            if(Mathf.Abs(180-link.transform.localRotation.eulerAngles.z) > (180-maxAngel))
            {
                link.transform.Rotate(new Vector3(0.0f, 0.0f, rotationZ * speed));
            }
